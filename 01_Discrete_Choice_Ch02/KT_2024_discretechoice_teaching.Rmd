...
KT_2024 <- read_csv("data/KinokoTakenokoSurvey_raw.csv")
...
KT_2024 <- KT_2024 %>%
  ...
  filter(gender != "3 : 回答したくない") %>% # 分析上扱いにくいデータを除外
...
datafig %>% 
    ...
    save_kable("output_WasedaIO_2024Spring/choice_table.tex") # tex形式で保存
```
...
rbind(gender_fig,adult_fig, fam_fig, region_fig)  %>%
  knitr::kable( digits = 2, "latex",linesep = c("", "\\addlinespace","", "\\addlinespace","","\\addlinespace","","","\\addlinespace"),
                booktabs = TRUE) %>%
  save_kable("output_WasedaIO_2024Spring/characteristics_table.tex")
```
# 分析
```{r}
# 分析用のデータ整形
...
```{r, results = "asis"}
# 表
stargazer::stargazer(ml,rcdc,ml_consumer_atr,rcdc_consumer_atr,column.labels=c("mlogit","rcdc","mlogit","rcdc"),single.row=TRUE,type="latex",align = TRUE)
...
plot(KT_dist_hist)
ggsave(filename = "output_WasedaIO_2024Spring/fig_hist_preference_KT.pdf", plot = KT_dist_hist, width = 6, height = 6)
...
ggsave(filename = "output_WasedaIO_2024Spring/fig_dist_preference_KT.pdf", plot = KT_dist_density, width = 6, height = 6)
```
...
ggsave(filename = "output_WasedaIO_2024Spring/fig_hist_price_coef.pdf", plot = p_coef_dist_hist, width = 6, height = 6)
...
ggsave(filename = "output_WasedaIO_2024Spring/fig_dist_price_coef.pdf", plot = p_coef_dist_density, width = 6, height = 6)
```
...
plot(fig_brand)
ggsave(filename = "output_WasedaIO_2024Spring/brand_value.pdf", plot = fig_brand, width = 12, height = 6)
...
```{r}
plot(fig_demand)
ggsave(filename = "output_WasedaIO_2024Spring/demand_Kinoko.pdf", plot = fig_demand, width = 8, height = 6)
```
...
```{r}
plot(fig_share)
ggsave(filename = "output_WasedaIO_2024Spring/share_kinoko.pdf", plot = fig_share, width = 8, height = 6)
```
...
plot(fig_rev)
ggsave(filename = "output_WasedaIO_2024Spring/Revenue_Kinoko.pdf", plot = fig_rev, width = 8, height = 6)
```
　