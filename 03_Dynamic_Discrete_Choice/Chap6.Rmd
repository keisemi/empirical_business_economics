...
data_gen %>%
  ...
  ggplot2::labs(y = str_wrap('頻 度', width = 1), x = '走行距離（百万km）')
...
data_gen %>%
  ...
  ggplot2::labs(y = str_wrap('頻 度', width = 1), x = '価格（百万円）')+
  scale_x_continuous(breaks = seq(2.0, 2.5, by = 0.1))
...
data_gen %>% 
  ...
  ggplot2::ggplot(aes(x = mileage, y = prob_buy)) + 
  ...
  ggplot2::labs(y = str_wrap('購 入 確 率', width = 1), x = '走行距離（百万km）')
...
data_gen %>% 
  ...
  dplyr::summarize(num_state = n(),
                   ...
                   .groups = 'drop') %>% 
  dplyr::mutate(prob_buy = sum_action / num_state) %>% 
  ggplot2::ggplot(aes(x = price, y = prob_buy)) + 
  ggplot2::geom_bar(stat = 'identity') +
  ggplot2::theme_bw() +
  ggplot2::theme(
    axis.title.y = element_text(angle = 0, vjust = 0.5),
  ...
  ggplot2::labs(y=str_wrap('購 入 確 率', width = 1), x = '価格（百万円）')
...
hist3D(x = mileage_states, y = price_states, z = t(prob_buy_obs_mat),
       zlim = c(0, 0.8),
       bty = 'g', phi = 10,  theta = -60, axes = TRUE, label = TRUE,
       xlab = '走行距離（百万km）', ylab = '価格（百万円）', zlab = '購入確率',
