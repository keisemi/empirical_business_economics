...
x_ANA = read.csv("data/x_ANA.csv")
x_JAL = read.csv("data/x_JAL.csv")
id_port_dyad_long = read.csv("data/id_port_dyad_long.csv")

#whether two dfs are identical
identical(select(x_ANA,-Flight), select(x_JAL,-Flight))
#True
...
# 4. check equilibrium multiplicity & model fit of 1st stage
```{r}
#est2_2_gmm2
df_long = df_long %>% 
  mutate(
...
# 5. counterfactual: 北陸新幹線の通る空港
```{r}
#counterfactual including 伊丹
df_long = df_long %>% 
