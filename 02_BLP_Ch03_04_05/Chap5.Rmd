...
```{r message = FALSE}
data <- readr::read_csv(file = "intermediate/chap5_data.csv")
...
est_table %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap5_blp.tex")
```
...
elasticity <- f_elasticity(
  datalist = datalist, parameter = parameter,
...
Table_4dt %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap5_mc_2016.tex")
```
...
**留意点**: 今回第５章では、日評自動車はホンダの車種を生産・販売する企業として扱う。
...
Table_5dt <- data_2016 %>%
  ...
  dplyr::select(
    Maker, Name, p_NippyoA_PPC,
    share_NippyoA_PPC, p_NippyoB_PPC, share_NippyoB_PPC
  ) %>%
...
Table_5dt %>%
  ...
  kableExtra::add_header_above(c(" ", " ", "Nippyo and Brand A" = 2, "Nippyo and Brand B" = 2)) %>%
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap5_merger_change_p_q.tex")
```
...
Table_6dt %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap5_merger_change_mc.tex")
```
...
pro_rev_2016 <- f_profit(
  data_2016$Maker, data_2016$price, data_2016$mc, data_2016$share, HH_2016
)  

pro_rev_NippyoA <- f_profit(
  data_2016$Maker, data_2016$p_NippyoA, data_2016$mc, data_2016$share_NippyoA, HH_2016
) 

pro_rev_NippyoB <- f_profit(
  data_2016$Maker, data_2016$p_NippyoB, data_2016$mc, data_2016$share_NippyoB, HH_2016
) 
...
Table_7dt1 %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap5_merger_welfare.tex")
```
...
Table_7dt2 %>%
  ...
  kableExtra::add_header_above(c(" ", "Nippyo and Brand A" = 2, "Nippyo and Brand B" = 2)) %>%
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap5_merger_revenue.tex")
```
...
p_NippyoA_TSfix <- f_eqprice(
  datalist_2016,data_2016$p_NippyoA,Ownership_NippyoA,
...
data_2016 <- data_2016 %>%
  dplyr::mutate(p_NippyoA_TSfix = p_NippyoA_TSfix, share_NippyoA_TSfix = share_NippyoA_TSfix)
```
...
p_NippyoB_TSfix <- f_eqprice(
  datalist_2016, data_2016$p_NippyoB, Ownership_NippyoB,
...
data_2016 <- data_2016 %>%
  dplyr::mutate(p_NippyoB_TSfix = p_NippyoB_TSfix, share_NippyoB_TSfix = share_NippyoB_TSfix)
...
Table_7dt3 %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap5_appendix_merger_mc.tex")
```
...
pro_rev_NippyoA_rc <- f_profit(
  data_2016$Maker, data_2016$p_NippyoA_TSfix, data_2016$mc_NippyoA_TSfix,
  data_2016$share_NippyoA_TSfix, HH_2016
) 

# 日評自動車とブランドB社が合併した場合の利益の導出
pro_rev_NippyoB_rc <- f_profit(
  data_2016$Maker, data_2016$p_NippyoB_TSfix, data_2016$mc_NippyoB_TSfix,
...
Table_7dt4 %>%
  ...
  kableExtra::add_header_above(c(" ", "Nippyo and Brand A" = 2, "Nippyo and Brand B" = 2)) %>%
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap5_appendix_merger_revenue.tex")
```
