################################################################################
#
# Aguirregabiria and Mira (2007)のモデルに基づいた経済セミナーのモデルの推定
# main.R
# Editor: Kodai Yasuda
# Last edited: 2024-7-5
#
################################################################################


# 1. はじめに----
# このスクリプトは経済セミナー第11回の資料を拡張し、Aguirregabiria and Mira (2007)のモデルに基づいた経済セミナーのモデルの推定を行うものである。
...
# 必要な関数の読み込み
functionlt <- list.files("./code/functions", # この部分は経済セミナーから変更されている
...
while (i > 0) {
  ...
  obj <- function(x){obj_lik(c(x[1], x[3:4], 0, x[5], x[2],x[3:4], 0, x[5]),
                             ccp1, ccp2,
...
# 結果のSummary
# CCP player 1
Summary_CCP1 <- matrix(c(CCP1UpdatedMat[,2], ccp1, apply(bootresult_CCP1,1,sd,na.rm = TRUE)), ncol = 3)
colnames(Summary_CCP1) <- c("CCP for firm 1: True", "Estimated", "SE")
print(Summary_CCP1)

# CCP player 2
Summary_CCP2 <- matrix(c(CCP2UpdatedMat[,2], ccp2, apply(bootresult_CCP2,1,sd,na.rm = TRUE)), ncol = 3)
