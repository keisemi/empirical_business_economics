fP_a1given <- function(Matrix1, Vec2){
  # input: 
  # 2 x 2 exogenous state transition matrix (Matrix1); 
  # 8 x 1 CCP essence vector of firm 2(Vec2)
  # output:
  # list of 3 8x8 state transition matrices
  # some lines are same as in fP funciton; 
  # adjustnment matrix multiplication rules out some impossible transitions
  
  # exogenous state transition probabilities; expansion to 8x8
  TempMat0 <-
    kronecker(Matrix1, matrix(rep(1, 16), ncol = 4))
  
  TempMat2 <- matrix(c(Vec2[1], 1-Vec2[1], Vec2[1],1-Vec2[1],
                       1-Vec2[2], Vec2[2],1-Vec2[2], Vec2[2],
                       Vec2[3], 1-Vec2[3], Vec2[3],1-Vec2[3], 
                       1-Vec2[4], Vec2[4], 1-Vec2[4],Vec2[4], 
                       Vec2[5], 1-Vec2[5],Vec2[5], 1-Vec2[5],
                       1-Vec2[6], Vec2[6],1-Vec2[6], Vec2[6],
                       Vec2[7], 1-Vec2[7], Vec2[7],1-Vec2[7], 
                       1-Vec2[8], Vec2[8], 1-Vec2[8],Vec2[8] ), ncol = 4, byrow = TRUE)
  TempMat2 <- cbind(TempMat2, TempMat2)
  
  # when a_2 = -1
  # 
  MatAdjustMinus1 <- matrix( rep(rep(c(0, 1), each = 16),2), nrow =8 ,byrow = TRUE) 
  MatAdjustMinus2 <- matrix( rep(rep(c(1, 0), each = 16),2), nrow =8 ) 
  output1 <- TempMat0 * TempMat2 * MatAdjustMinus1 * MatAdjustMinus2
  
  # when a_2 = 0
  ForZero <- matrix(c(1, 0, 0, 1), ncol = 2 ,byrow = TRUE) 
  MatAdjustZero <- kronecker( ForZero , matrix(rep(1, 4), ncol = 2))
  MatAdjustZero <- cbind(MatAdjustZero, MatAdjustZero)
  MatAdjustZero <- rbind(MatAdjustZero, MatAdjustZero)
  output2 <- TempMat0 * TempMat2 * MatAdjustZero
  
  # when a_2 = 1
  MatAdjustPlus1 <- matrix(rep(rep(c(1, 0), each = 16),2), nrow =8 , byrow = TRUE ) 
  MatAdjustPlus2 <- matrix(rep(rep(c(0, 1), each = 16),2), nrow =8 ) 
  output3 <- TempMat0 * TempMat2 * MatAdjustPlus1 * MatAdjustPlus2
  
  output <- list(output1, output2, output3)
  
  output
}
