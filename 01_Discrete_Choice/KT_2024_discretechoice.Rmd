...
# 各個人の属性のパラメタを計算して平均する→その値と選好パラメタ(消費者属性を考慮していない)の平均との和を取る
...
```
```{r}
alpha_Kinoko_for_dist <- NULL
alpha_Takenoko_for_dist <- NULL
beta_price_for_dist <- NULL
for(i in 1:236){
set.seed(100+i)

alpha_Kinoko_draw = dist_Kinoko$sigma*rnorm(1000, mean = 0, sd = 1)+param_Kinoko[i]
alpha_Kinoko_for_dist = c(alpha_Kinoko_for_dist, alpha_Kinoko_draw)

alpha_Takenoko_draw = dist_Takenoko$sigma*rnorm(1000, mean = 0, sd = 1)+param_Takenoko[i]
alpha_Takenoko_for_dist = c(alpha_Takenoko_for_dist, alpha_Takenoko_draw)

beta_price_draw = dist_price$sigma*rnorm(1000, mean = 0, sd = 1)+param_price[i]
beta_price_for_dist = c(beta_price_for_dist, beta_price_draw)
}
```
...
data_for_distribution =
  tibble( alpha_Kinoko_for_dist , 
          alpha_Takenoko_for_dist ,  
          beta_price_for_dist) 
# きのことたけのこの係数の差の分布
KT_dist_hist <- ggplot(data = data_for_distribution,aes(x = alpha_Kinoko_for_dist - alpha_Takenoko_for_dist)) +
...
KT_dist_density <- ggplot(data = data_for_distribution,aes(x = alpha_Kinoko_for_dist - alpha_Takenoko_for_dist)) +
  geom_density() +
...
```{r}
# 価格係数の分布
p_coef_dist_hist <- ggplot(data = data_for_distribution, aes(beta_price_for_dist)) +
...
p_coef_dist_density <- ggplot(data = data_for_distribution, aes(beta_price_for_dist)) +
  geom_density()+
...
# 支払い意思額の計算
data_for_distribution %>%
  mutate( brand_Kinoko = alpha_Kinoko_for_dist / beta_price_for_dist, 
          brand_Takenoko = alpha_Takenoko_for_dist / beta_price_for_dist,
          love_Kinoko = ifelse(alpha_Kinoko_for_dist > alpha_Takenoko_for_dist, 1, 0 )) -> data_for_distribution 
