...
```{r}
# 初期パラメタ
ini <- c(20, 20, 0.1, 3, 3,-0.1,-0.1,-3, -3, 7, 7, 7, 7, 0, 0, 0, 0, 0)
...
# 推定値の取得
est_vec <- as.numeric(result_with_atr[1,1:17])
```
...
```{r}
# テスト用関数
f_likelihood_logit_with_atr_test <- function(param, data){
  pred <- data.frame(matrix(ncol = 3, nrow = 1180))
  colnames(pred) <- c("prob_Kinoko", "prob_Takenoko","prob_Other")
...
# 各選択フェーズでの選択確率を予測
  ...
  data %>%
    ...
    mutate( choice_prob = case_when( choice == 0 ~ prob_Other,
                                     choice == 1 ~ prob_Kinoko,
                                     choice == 2 ~ prob_Takenoko) ) %>%
    mutate(log_choice_prob = log(choice_prob)) ->　testdf
　
  return(testdf)
...
```{r}
f_likelihood_logit_with_atr_test(ini, data_for_estimation_with_atr)
```
