...
```
　以下が推定されたパラメタである。パッケージによる推定に近い値となっていることがわかる。
```{r}
# パッケージの推定値
...
```
　また、以下が対数尤度である。こちらもほとんど変わらないことが見て取れる。
```{r}
...
　はじめに、扱いやすくするためにデータの一部変数をダミー変数に直しておく。
```{r}
# ダミー変数に直す
...
f_logit_prob_with_atr <- function(alpha_Kinoko, alpha_Takenoko, beta, price_Kinoko, price_Takenoko, param_atr_Kinoko,param_atr_Takenoko, param_atr_price,atr){
# きのこ、たけのこから得られるそれぞれの効用の算出
...
# 各選択フェーズでの選択確率を予測
  P1 <- f_logit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 200, 200, param_atr_Kinoko,param_atr_Takenoko, param_atr_price,atr)
  P2 <- f_logit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 180, 200, param_atr_Kinoko,param_atr_Takenoko, param_atr_price,atr)
  P3 <- f_logit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 200, 170, param_atr_Kinoko,param_atr_Takenoko, param_atr_price,atr)
  P4 <- f_logit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 220, 200, param_atr_Kinoko,param_atr_Takenoko, param_atr_price,atr)
  P5 <- f_logit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 190, 210, param_atr_Kinoko,param_atr_Takenoko, param_atr_price,atr)
  # 格納
  pred[(5*(i-1)+1),] <- P1
  pred[(5*(i-1)+2),] <- P2
  pred[(5*(i-1)+3),] <- P3
...
# 初期パラメタ
# おおざっぱに決めた初期値
ini <- c(0, 10, 10, 0, 1, 1, 0, 0, 0, 0, -1, -1, 0, 0, 1, 1, 1, 1)


# 最適化 Nealder-Meadでは初期値にかなり依存して推定されてしまうため(おそらくパラメタの多さによるもの)、BFGSを用いる。
...
```
　推定された値をmlogitパッケージによる推定値と比べると以下のようになっており、概ね近い値となっている。
```{r}
# パッケージでの推定値:
...
## ランダム係数ロジット(消費者属性無し)
　続いて、ランダム係数ロジットをスクラッチで推定する。まず、以下でドロー回数分の個人の選択確率を算出する。
　
```{r}

f_rcdclogit_prob <- function(alpha_Kinoko, alpha_Takenoko, beta, price_Kinoko, price_Takenoko, sigma_Kinoko, sigma_Takenoko, sigma_price, tau){
# きのこ、たけのこから得られるそれぞれの効用の算出
...
　方針:各回答者ごとの一連の選択の確率を1000ドローしたものを、平均して近似する。以下で対数尤度を算出する。
```{r}
f_likelihood_rcdclogit <- function(param, data){
  # 空のデータフレーム
...
# 各選択フェーズでのドロー回数分の選択確率を予測
  P1 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 200, 200, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  P2 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 180, 200, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  P3 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 200, 170, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  P4 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 220, 200, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  P5 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 190, 210, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  ...
  data %>%
    ...
    select(log_choice_joint_prob) -> result
...
```{r}
# パッケージの推定値
est_vec_package_rcdc <- rcdc$coefficients[1:6]
# 比較
print(rbind(est_vec_rcdc, est_vec_package_rcdc))
```
　対数尤度を以下に示す。こちらも概ね同じである。
```{r}
...
```
## ランダム係数ロジット(消費者属性入り)
　次に、消費者属性入りのランダム係数ロジットをスクラッチで推定する。消費者属性を含める以外は、先ほどの属性なしのものと変わらない。


　まず、以下で選択確率を定義する。
```{r}

f_rcdclogit_prob_with_atr <- function(alpha_Kinoko, alpha_Takenoko, beta, price_Kinoko, price_Takenoko, param_atr_Kinoko,param_atr_Takenoko, param_atr_price, atr, sigma_Kinoko, sigma_Takenoko, sigma_price, tau){
# きのこ、たけのこから得られるそれぞれの効用の算出
# sigma_〇〇は各パラメタの標準偏差、tauは標準正規分布に従う乱数を(ドロー回数)行×3列発生させたもの
  alpha_Kinoko_draw <- alpha_Kinoko + sigma_Kinoko*tau[, 1]
  alpha_Takenoko_draw <- alpha_Takenoko + sigma_Takenoko*tau[, 2]
...
　以下で対数尤度を算出する。
```{r}
f_likelihood_rcdclogit_with_atr <- function(param, data){
  # 空のデータフレーム
  pred <- matrix(ncol = 3, nrow = 1180*times_draw)
  colnames(pred) <- c("prob_Kinoko", "prob_Takenoko","prob_Other")
  ID <- numeric(236*times_draw)
...
# 各選択フェーズでのドロー回数分の選択確率を予測
  P1 <- f_rcdclogit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 200, 200, param_atr_Kinoko,param_atr_Takenoko, param_atr_price, atr, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  P2 <- f_rcdclogit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 180, 200, param_atr_Kinoko,param_atr_Takenoko, param_atr_price, atr, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  P3 <- f_rcdclogit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 200, 170, param_atr_Kinoko,param_atr_Takenoko, param_atr_price, atr, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  P4 <- f_rcdclogit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 220, 200, param_atr_Kinoko,param_atr_Takenoko, param_atr_price, atr, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  P5 <- f_rcdclogit_prob_with_atr(alpha_Kinoko, alpha_Takenoko, beta, 190, 210, param_atr_Kinoko,param_atr_Takenoko, param_atr_price, atr, sigma_Kinoko, sigma_Takenoko, sigma_price, tau[(times_draw*(i-1)+1):(times_draw*i), ])
  # 各設問ごとのドロー回数分の選択確率を格納
  pred[(5*(i-1)*(times_draw)+1):(5*(i-1)*(times_draw)+times_draw),] <- P1
  pred[(5*(i-1)*(times_draw)+times_draw+1):(5*(i-1)*(times_draw)+2*times_draw),] <- P2
  pred[(5*(i-1)*(times_draw)+2*times_draw+1):(5*(i-1)*(times_draw)+3*times_draw),] <- P3
