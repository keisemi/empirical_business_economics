...
print(est_vec)
```
　mlogitパッケージによる推定値と比べると以下のようになっている。概ね近い値となっている。
```{r}
# パッケージでの推定値:
est_vec_pack <- c(0.063, 12.521,12.661,0.013, 3.363,3.529, -0.002, -0.046 ,-0.429, -0.018, -3.324,	-3.664,0.016, 0.013, 3.817,2.796, 3.365,3.390 )
# 比較 やや差があるが概ね同じ
...
```{r}
# 対数尤度はそれぞれこのようになっている。
print(result_with_atr[19]$value)
ml_consumer_atr$logLik
```
　標準誤差を算出する。
```{r}
#　ヘシアンの計算
...
```{r}
f_likelihood_rcdclogit <- function(param, data, times_draw){
  # 空のデータフレーム
  pred <- data.frame(matrix(ncol = 3, nrow = 1180*times_draw))
  colnames(pred) <- c("prob_Kinoko", "prob_Takenoko","prob_Other")
  ...
  tau<- matrix(nrow = times_draw, ncol = 3)

# 各回答者の選択確率を予測する
  for(i in 1:236){
  set.seed(500+i)
  # 回答者iがドローする乱数
  tau[, 1] <- rnorm(times_draw, mean = 0, sd = 1)
  tau[, 2] <- rnorm(times_draw, mean = 0, sd = 1)
  tau[, 3] <- rnorm(times_draw, mean = 0, sd = 1)
# 各選択フェーズでのドロー回数分の選択確率を予測
  P1 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 200, 200, sigma_Kinoko, sigma_Takenoko, sigma_price, tau)
  P2 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 180, 200, sigma_Kinoko, sigma_Takenoko, sigma_price, tau)
  P3 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 200, 170, sigma_Kinoko, sigma_Takenoko, sigma_price, tau)
  P4 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 220, 200, sigma_Kinoko, sigma_Takenoko, sigma_price, tau)
  P5 <- f_rcdclogit_prob(alpha_Kinoko, alpha_Takenoko, beta, 190, 210, sigma_Kinoko, sigma_Takenoko, sigma_price, tau)
  ...
  ID[(5*(i-1)*(times_draw)+1):(5*(i-1)*(times_draw)+5*times_draw)] <- rep(data$ID[(5*(i-1)+1)], 5*times_draw)
...
```
```{r}
# カンニングしてパッケージで推定されたパラメタを入れたら、対数尤度は同じくらいになった
f_likelihood_rcdclogit( c(0.164, 35.194, 36.499, -0.031, 1.238, 4.294), data_for_estimation_with_atr, 50)
```
