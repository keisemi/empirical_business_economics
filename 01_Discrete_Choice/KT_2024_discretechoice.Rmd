...
alpha_Takenoko_mean <- dist_Takenoko$mean + mean(param_Takenoko_atr)
...
alpha_Kinoko_mean <- dist_Kinoko$mean + mean(param_Kinoko_atr)
...
beta_price_mean <- dist_price$mean + mean(param_price_atr)
```
...
KT_dist <- ggplot(data = data.frame(x = c(-25, 25)), aes(x)) +
  stat_function(fun = dnorm, n = 101, 
                args = list(mean = alpha_Kinoko_mean -alpha_Takenoko_mean , 
...
p_coef_dist <- ggplot(data = data.frame(x = c(0.05, 0.30) ), aes(x)) +
  stat_function(fun = dnorm, n = 101, 
                args = list(mean = beta_price_mean, sd = abs(dist_price$sigma)), colour = "red" ) + 
