...
tbl_ols_intro %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap3_ols_intro.tex")
```
...
tbl_data_summary %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap3_data_summary.tex")
```
...
tbl_logit_iv %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap3_logit_iv.tex")
```
...
tbl_logit_iv_1st %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap3_logit_iv_1st.tex")
```
...
tbl_own_elas %>%
  ...
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::save_kable("output/chap3_own_elas.tex")
```
...
```{r}
f_share <- function(price_cand, year, modelID_target, dt, est_param) {
  
  dt_result <- dt %>% 
    dplyr::filter(year == 2016) %>% 
    dplyr::mutate(temp_price = ifelse(NameID == modelID_target, price_cand, price)) %>% 
    dplyr::mutate(
...
```{r}
# 最適化で利用する関数を定義する。
f_revenue <- function(price_cand, year, modelID_target, dt, est_param) {
  
  quantity <- f_share(price_cand, year, modelID_target, dt, est_param)
...
result <- optimise(
  f_revenue, interval = c(0.3, 3), maximum = TRUE, 
  year = 2016, modelID_target = NameID_target,
  dt = dt_application, est_param = estparam
)
print(result)
```