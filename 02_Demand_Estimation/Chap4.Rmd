...
```{r}
elas_mat_restricted <- elas_mat[NameID %in% c(87, 122, 155, 178), NameID %in% c(87, 122, 155, 178)]
...
elas_mat_restricted %>%
  ...
  kable_styling(full_width = FALSE) %>%
  save_kable("output/chap4_elas_mat_logit.tex")
```
...
ols <-
  fixest::feols(
    logit_share ~ price + log_inside_share + hppw + FuelEfficiency + size + 
      capacity_d + FuelRegular_d | year,
    data = data
  )
...
alpha2 <- iv_GH$coefficients["fit_price"]
sigma2 <- iv_GH$coefficients["fit_log_inside_share"]

data %>% 
  dplyr::mutate(
    own_elas_ols = alpha1 * price * (1 - sigma1 * inside_share - (1 - sigma1) * share) / (1 - sigma1),
    own_elas_ivgh = alpha2 * price * (1 - sigma2 * inside_share - (1 - sigma2) * share) / (1 - sigma2)
  ) -> data_elas
