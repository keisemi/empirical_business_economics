...
data_NIPPYO %>% 
  dplyr::filter(year == 2016) %>% 
  dplyr::select(price, share, NameID) %>% 
  dplyr::arrange(NameID) -> dt2016
...
data <-
  ...
  dplyr::mutate(
    dplyr::across(c("hppw", "FuelEfficiency", "size", "FuelRegular_d", "capacity_d"),
                  list(sum_own = ~ sum(.x, na.rm = TRUE))),
    dplyr::across(c("hppw", "FuelEfficiency", "size"),
                  list(sqr_sum_own = ~ sum(.x^2, na.rm = TRUE))),
    group_n = n()
...
# ダミー変数を最後の列に移動
data <- data %>%
  relocate(ends_with("_d"), starts_with("year_"), .after = last_col())

# データの保存(第５章で使用)
readr::write_csv(data, "intermediate/chap4_data.csv")
```

```{r}
# 入れ子型ロジットでは、外国車ダミー(Foreign_d)を除いている。

# OLSの結果
ols <-
  fixest::feols(logit_share ~ price + log_inside_share + hppw + 
                  FuelEfficiency + size + capacity_d + FuelRegular_d | year ,
                data = data)

# BLP操作変数を用いた結果
iv_BLP2 <-
  fixest::feols(
    logit_share ~ hppw + FuelEfficiency + size + FuelRegular_d + capacity_d | year |
      price + log_inside_share ~ iv_BLP_own_hppw_nest + iv_BLP_own_FuelEfficiency_nest +
      iv_BLP_own_size_nest + iv_BLP_other_hppw_nest + iv_BLP_other_FuelEfficiency_nest +
...
# 推定結果をレポートする。
tbl_nest_iv <- fixest::etable(list("OLS" = ols, "IV_BLP" = iv_BLP2, "IV_GH" = iv_GH),  
                              se = "hetero",
                              fitstat = c("r2", "n", "ivf"), 
                              signif.code = NA, 
                              dict = c(price = "自動車価格",
...
tbl_nest_iv %>%
  ...
  kable_styling(full_width = FALSE) %>%
  save_kable("output/chap4_nest_iv.tex")
```
...
alpha3 <- iv_GH$coefficients["fit_price"]
sigma3 <- iv_GH$coefficients["fit_log_inside_share"]
...
data %>% 
  dplyr::mutate(
    ...
    own_elas_ivgh = alpha3 * price * (1 - sigma3 * inside_share - (1 - sigma3) * share) / (1 - sigma3)) -> data_elas
...
tbl_nest_own_elas %>%
  kable(align = rep("l", 5),
        ...
        booktabs = FALSE, format = "latex") %>%
  kable_styling(full_width = FALSE) %>%
  save_kable("output/chap4_nest_own_elas.tex")
```
...
own_elas <- alpha3 * price * (1 - sigma3 * inside_share - (1 - sigma3) * share) / (1 - sigma3)

# Cross elasticity outside the group
# cross_elas_othergroup <- (-1) * alpha2 * price * share
cross_elas_othergroup <- (-1) * alpha3 * price * share
J <- length(own_elas)
...
cross_elas_samegroup <- (-1) * alpha3 * price_l_mat * (sigma3 * insideshare_l_mat + (1 - sigma3) * share_l_mat) / (1 - sigma3)
...
elas_mat_nl_restricted <- elas_mat_nl[NameID %in% c(87, 122, 155, 178), NameID %in% c(87, 122, 155, 178)]
...
elas_mat_nl_restricted %>%
  kable(align = rep("c", 4),
        ...
        booktabs = FALSE, format = "latex") %>%
  kable_styling(full_width = FALSE) %>%
  save_kable("output/chap4_elas_mat_nest.tex")
```
...
-\alpha_{i} p_{j t}+\xi_{j t}+\xi_t+\epsilon_{i j t}
\end{eqnarray}
$$
...
2. GMM目的関数のインプットは非線形パラメタ`theta2`のみである。これは、平均効用に線形の形で入ってくるパラメタ$\theta_1$は、非線形パラメタを所与とすると線形GMMで解析的に求まるためである。この点についてはウェブ付録・第４章のページにアップされた補足資料を参照されたい。
...
```{r}
beta_hat <- rbind(result2$beta_hat, result$par)
...
result_table %>%
  kable(align = c("l", rep("c", 2)),
        ...
        caption = "Estimates of The Parameters") %>%
  kable_styling(full_width = FALSE) %>%
  save_kable("output/chap4_blp.tex")
```
...
elas_mat_blp_restricted <- elasticity$elasmat_2016[NameID2016 %in% c(87, 122, 155, 178), NameID2016 %in% c(87, 122, 155, 178)]
...
elas_mat_blp_restricted %>%
  kable(align = rep("c", 4),
        ...
        booktabs = FALSE, format = "latex") %>%
  kable_styling(full_width = FALSE) %>%
  save_kable("output/chap4_elas_mat_blp.tex")
```
...
```{r}
f_revenue(price = optimprice3$maximum,
          data = data, datalist = datalist, result2 = result2,
          parameter = parameter, option = "totalpi")
```